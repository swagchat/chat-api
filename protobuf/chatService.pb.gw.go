...
var (
	filter_RoomUserService_GetUserIDsOfRoomUser_0 = &utilities.DoubleArray{Encoding: map[string]int{}, Base: []int(nil), Check: []int(nil)}
)

func request_RoomUserService_GetUserIDsOfRoomUser_0(ctx context.Context, marshaler runtime.Marshaler, client RoomUserServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	var protoReq GetUserIDsOfRoomUserReq
	var metadata runtime.ServerMetadata

	if err := runtime.PopulateQueryParameters(&protoReq, req.URL.Query(), filter_RoomUserService_GetUserIDsOfRoomUser_0); err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "%v", err)
	}

	msg, err := client.GetUserIDsOfRoomUser(ctx, &protoReq, grpc.Header(&metadata.HeaderMD), grpc.Trailer(&metadata.TrailerMD))
	return msg, metadata, err
...
func RegisterRoomUserServiceHandlerClient(ctx context.Context, mux *runtime.ServeMux, client RoomUserServiceClient) error {
	...
	mux.Handle("GET", pattern_RoomUserService_GetUserIDsOfRoomUser_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		ctx, cancel := context.WithCancel(req.Context())
		defer cancel()
		if cn, ok := w.(http.CloseNotifier); ok {
			go func(done <-chan struct{}, closed <-chan bool) {
		...
		resp, md, err := request_RoomUserService_GetUserIDsOfRoomUser_0(rctx, inboundMarshaler, client, req, pathParams)
		ctx = runtime.NewServerMetadataContext(ctx, md)
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
		...
		forward_RoomUserService_GetUserIDsOfRoomUser_0(ctx, mux, outboundMarshaler, w, req, resp, mux.GetForwardResponseOptions()...)
...
var (
	...
	pattern_RoomUserService_GetUserIDsOfRoomUser_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1}, []string{"roomUsers", "users"}, ""))
...
var (
	...
	forward_RoomUserService_GetUserIDsOfRoomUser_0 = runtime.ForwardResponseMessage
