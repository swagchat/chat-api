...
func rdbInsertRoomUsers(db string, roomUsers []*model.RoomUser) error {
	...
	trans, err := master.Begin()
	if err != nil {
		err := errors.Wrap(err, "An error occurred while inserting roomUsers")
		logger.Error(err.Error())
		return err
	}
...
func rdbSelectRoomUsersByRoomID(db, roomID string) ([]*model.RoomUser, error) {
	...
	_, err := replica.Select(&roomUsers, query, params)
	if err != nil {
		err := errors.Wrap(err, "An error occurred while getting roomUsers by roomId")
		logger.Error(err.Error())
		return nil, err
	}
...
func rdbSelectRoomUsersByUserID(db, userID string) ([]*model.RoomUser, error) {
	...
	_, err := replica.Select(&roomUsers, query, params)
	if err != nil {
		err := errors.Wrap(err, "An error occurred while getting roomUsers by userId")
		logger.Error(err.Error())
		return nil, err
	}
...
func rdbSelectUserIDsOfRoomUser(db string, roomID string, opts ...RoomUserOption) ([]string, error) {
	replica := RdbStore(db).replica()
